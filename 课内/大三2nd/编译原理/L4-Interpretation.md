...
* [抽象语法树：Abstract Syntax Tree](#抽象语法树abstract-syntax-tree)
  * [Fun表达式-AST例子](#fun表达式-ast例子)
  * [Fun命令 - AST例子](#fun命令---ast例子)
  * [Fun程序 - AST例子](#fun程序---ast例子)
  * [Fun变量声明&类型 - AST](#fun变量声明类型---ast)
  * [Fun过程 - AST](#fun过程---ast)
...
– **虚拟机状态**由一组**变量**来表示 the virtual machine state is represented by a group of variables
– 每条指令都是通过**检查和/或更新虚拟机状态**来执行的。 each instruction is executed by inspecting and/or updating the virtual machine state
...
A S → T compiler translates a source program in S to object code in T, if it conforms to the source language’s syntax and scope/type rules. S → T编译器将S中的源程序翻译成T中的对象代码，如果它符合源语言的语法和范围/类型规则
...
注意：AST没有区分exprs、sec-exprs等：它们都只是表达式。Note: The AST makes no distinction between exprs, sec-exprs, etc.: they are all just expressions.
